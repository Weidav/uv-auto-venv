...
suite('uv-auto-venv Extension Test Suite', function () {
	...
	test('Should debounce rapid editor changes', async () => {
		...
		// Wait for the debouncer to fire and the process to finish
		let activeEnv = api.environments.getActiveEnvironmentPath(vscode.Uri.file(fixturesFolder));
		const timeout = Date.now() + 10000;
		while (Date.now() < timeout) {
		...
		assert.ok(!intermediateEnvSeen, "Intermediate environments (app, bare) should not be set due to debouncing");
		assert.ok(activeEnv?.path?.includes('example-lib/.venv/bin'), `Expected path to include 'example-lib/.venv/bin', but got ${activeEnv?.path}`);
	});
});

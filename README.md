...
Starting with VS Code 2026, Microsoft is rolling out a new [Python Environments](https://marketplace.visualstudio.com/items?itemName=ms-python.vscode-python-envs) extension that replaces parts of the classic Python extension's environment management. When enabled via `python.useEnvironmentsExtension`, different VS Code components read the active interpreter from different sources:

- **debugpy / test discovery** → python-envs API
- **Pylance / linters** → classic `ms-python.python` API

**uv Auto venv v1.5.0+ handles this automatically:**

| Mode                                                              | How it works                                                                                                                                                                                                                                    |
| ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Classic** (`python.useEnvironmentsExtension: false` or absent)  | Sets the interpreter via the classic `ms-python.python` API only.                                                                                                                                                                               |
| **Python Environments** (`python.useEnvironmentsExtension: true`) | Sets the interpreter via **both** the `ms-python.vscode-python-envs` API (for debugpy / tests) and the classic API (for Pylance / linters). The python-envs scope is per file for PEP 723 scripts and per workspace folder for normal projects. |

The extension detects which mode is active at startup. If the Python Environments extension is not installed, only the classic API is used.
...
4. When `python.useEnvironmentsExtension` is enabled, the extension sets the interpreter through **both** the python-envs API (so debugpy and test discovery use the correct environment) and the classic API (so Pylance and linters pick it up too). The python-envs scope is per-file for PEP 723 scripts.
